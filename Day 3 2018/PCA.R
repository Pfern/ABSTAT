...
# Exercise 3 - Iris 
# ---------------------------------
data(iris)   #continuos variables are ont the same units
?iris
mode(iris)
dim(iris)
str(iris)
colnames(iris)
class(iris)
data<-iris[,1:4]  #create an object just with the quantitative data

plot(data,col=specol)

?princomp

pca.iris<-princomp(data)

names(pca.iris)

pca.iris$sdev  # square root eigenvalues

eigenva<-pca.iris$sdev^2

varexp<-eigenva/sum(eigenva)

varexpcum<-cumsum(varexp)

print(varexpcum)

load<-pca.iris$loadings
pca.iris$scores
plot(pca.iris)


plot(pca.iris$sdev,type="lines",main="Scree plot") # scree plot
biplot(pca.iris)
...
spe <- iris$Species
specol <-c(rep("blue",50),rep("red",50),rep("green",50))
points(pca.iris$scores[,1]/pca.iris$sdev[1],pca.iris$scores[,2]/pca.iris$sdev[2],pch=16,col="green")


###########################   prcomp function
scale.data<-scale(data,center=T)
iris.pca<-prcomp(scale.data)
summary(iris.pca)
(iris.pca$sdev)^2    #Kaiser's criterion: retain PC with variance >1
                     # for standardized data
screeplot(iris.pca)  # retain PCs with cumulative variance around 90%
 
iris.pca$rotation    #loadings

plot(iris.pca$x[,1],iris.pca$x[,2])  #make a scatterplot
text(iris.pca$x[,1],iris.pca$x[,2],rownames(iris),cex=0.7,pos=4,col="red")
